...
struct tj_time* tj_time_convert(int input_time)
{
	...
	int input_time0 = input_time + 28800;   //UTC+8时区,加上8*60*60秒
	int day_month[12] = { 31, 28, 31, 30, 31, 30, 31, 31, 30, 31, 30, 31 };
	int seconds = input_time0 % 60;
	input_time0 /= 60;
	int minutes = input_time0 % 60;
	input_time0 /= 60;
	int hours = input_time0 % 24;
	int days = input_time0 / 24;
	...
	while (days > 0) 
	{
		...
		// 如果当前月份的天数加上之前的剩余天数小于或等于一个月的天数，则更新天数和月份
		if (days < day_month[month]) {
			day += days;
			break;
		}
		days -= day_month[month];
		month = (month + 1) % 12;
		if (month == 0) 
			year++;
	}
