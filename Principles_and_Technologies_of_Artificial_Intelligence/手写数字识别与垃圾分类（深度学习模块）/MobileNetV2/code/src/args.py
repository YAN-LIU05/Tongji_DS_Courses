...
def eval_parse_args():
    ...
    eval_parser.add_argument('--platform', type=str, default="CPU", choices=("Ascend", "GPU", "CPU"), \
        help='run platform, only support GPU, CPU and Ascend')
    eval_parser.add_argument('--dataset_path', type=str, default="./data", required=True, help='Dataset path')
    eval_parser.add_argument('--pretrain_ckpt', type=str, required=True, default="",help='Pretrained checkpoint path \
        for fine tune or incremental learning')
