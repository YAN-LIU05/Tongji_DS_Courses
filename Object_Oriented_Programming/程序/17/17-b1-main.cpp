/* 2352018 大数据 刘彦 */
/* 2250898 陆思宇 2352201 燕欣睿 2351044 崔艺洋 */
...
//		cout << "s1应为HelloHello!HelloHello!HelloHello!，实际输出：" << (((s1 *= 2) + '!') *= 3) << endl;
	...
	if (1) {
		TStringAdv s1("oop"), s2, s3;

		cout << "assign函数测试" << endl;

		s1.assign("面向对象的程序设计");
		cout << "s1应为面向对象的程序设计，    实际输出：" << s1 << endl;

		s1.assign("oop");
		cout << "s1应为oop，                   实际输出：" << s1 << endl;

		s2.assign("OOP");
		cout << "s2应为OOP，                   实际输出：" << s2 << endl;

		s2.assign("");
		cout << "s2应为<EMPTY>，               实际输出：" << s2 << endl;

		const char* str = "大作业!!!";
		s1.assign("hello").assign(str);
		cout << "s1应为大作业!!!，             实际输出：" << s1 << endl;

		wait_for_enter();
	}

	if (1) {
		TStringAdv s1("Love"), s2, s3;
		const char* str1 = "you";
		char str2[] = "!!!";
		char c = '*';
		char empty_pos = '\0';
		...
		s1.append(c);
		cout << "s1应为Love*，       实际输出：" << s1 << endl;

		s1.append(empty_pos);
		cout << "s1应为Love*，       实际输出：" << s1 << endl;

		s1.append(str1);
		cout << "s1应为Love*you，    实际输出：" << s1 << endl;

		s2.append(str2);
		cout << "s2应为!!!，         实际输出：" << s2 << endl;

		s3.append(s1).append(s2);
		cout << "s3应为Love*you!!!， 实际输出：" << s3 << endl;

		wait_for_enter();
	}

	if (1) {
		TStringAdv s1 = "This is tongji", s2 = "Computerprgram ", s3;
		const char* str1 = " our";
		char c = '!';
		...
		s1.insert(str1, 8);
		cout << "s1应为This is our tongji，                    实际输出：" << s1 << endl;

		s1.insert(c, 19);
		cout << "s1应为This is our tongji!，                   实际输出：" << s1 << endl;

		s2.insert(' ', 9).insert('o', 12);
		cout << "s2应为Computer program，                      实际输出：" << s2 << endl;

		s1.insert(s2, 6);
		cout << "s1应为This Computer program is our tongji!，  实际输出：" << s1 << endl;

		s3.insert(s1, 1);
		s3.insert("oop", 30).insert('\0', 33);
		cout << "s3应为This Computer program is our oop，      实际输出：" << s3 << endl;

		wait_for_enter();
	}

	if (1) {
		TStringAdv s1 = "love", s2 = "love_u love_u _ulove _lu", s3 = "love";
		char  str1[] = "love";
		const char* str2 = "_u";
		char c = 'l', c1 = ' ';
		...
		s1.erase(str2);
		cout << "s1应为love，                    实际：" << s1 << endl;

		s1.erase(str1);
		cout << "s1应为<EMPTY>，                 实际：" << s1 << endl;

		s1.erase(c);
		cout << "s1应为<EMPTY>，                 实际：" << s1 << endl;

		s2.erase(s3);
		cout << "s2应为_u love_u _ulove _lu，    实际：" << s2 << endl;

		s2.erase(str1);
		cout << "s2应为_u _u _ulove _lu，        实际：" << s2 << endl;

		s2.erase(str2);
		cout << "s2应为 _u _ulove _lu，          实际：" << s2 << endl;

		s2.erase(str2);
		cout << "s2应为  _ulove _lu，            实际：" << s2 << endl;

		s2.erase(str1);
		cout << "s2应为  _u _lu，                实际：" << s2 << endl;

		s2.erase(str1);
		cout << "s2应为  _u _lu，                实际：" << s2 << endl;

		s2.erase(str2);
		cout << "s2应为   _lu，                  实际：" << s2 << endl;

		s2.erase(str2);
		cout << "s2应为   _lu，                  实际：" << s2 << endl;

		s2.erase(c1).erase(c1).erase(c1);
		cout << "s2应为_lu，                     实际：" << s2 << endl;

		s2.erase(str2);
		cout << "s2应为_lu，                     实际：" << s2 << endl;

		s2.erase(c);
		cout << "s2应为_u，                      实际：" << s2 << endl;

		s2.erase(str2);
		cout << "s2应为<EMPTY>，                 实际：" << s2 << endl;
