...
{
	if (1) {
		const TString s1;                 //s1为空
		const TString s2 = nullptr;        //s2为空
		const TString s3(nullptr);         //s3为空
		const TString s4 = "";            //s4为空
		const TString s5("");             //s5为空
		const TString s6("teststr2");      //s6为"teststr2"
		const TString s7 = "teststr3";     //s7为"teststr3"
	...
	if (1) {
		...
		cout << "自连接(+=)简单测试(TString类和TString类)" << endl;
	...
	if (1) {
		...
		cout << "自连接(+=)简单测试(TString类和字符指针、字符数组、字符串常量、字符常量)" << endl;
	...
	if (1) {
		...
		cout << "append()简单测试(TString类和TString类)" << endl;
	...
	if (1) {
		...
		cout << "append()简单测试(TString类和字符指针、字符数组、字符串常量、字符常量)" << endl;
...
//		cout << "s1应为HelloHello!HelloHello!HelloHello!，实际输出：" << (((s1 *= 2) + '!') *= 3) << endl;
	...
	if (1) {
		...
		cout << "比较运算测试(TString类和字符串常量1)" << endl;
	...
	if (1) {
		...
		cout << "比较运算测试(TString类和字符串常量2)" << endl;
		/* 空的TString和nullptr、"" 都认为相等（本质上，nullptr是地址，""是串，没有可比性，但是此处做特殊处理） */
		cout << "串1=" << s1 << " 串2=" << s2 << "    > 结果应为0，实际：" << (s1 > s2) << endl;
		cout << "串1=" << s1 << " 串2=" << s2 << "    >=结果应为1，实际：" << (s1 >= s2) << endl;
		cout << "串1=" << s1 << " 串2=" << s2 << "    < 结果应为0，实际：" << (s1 < s2) << endl;
		cout << "串1=" << s1 << " 串2=" << s2 << "    <=结果应为1，实际：" << (s1 <= s2) << endl;
	...
	if (1) {
		...
		cout << "比较运算测试(TString类和字符数组)" << endl;
