...
# 逻辑回归训练函数
def train_logistic_regression(train_data, y_train, params=None):
    """使用逻辑回归进行训练"""
    if params is None:
        # 逻辑回归的一些常用参数
        params = {
...
# 调用函数进行训练
logistic_model, auc = train_logistic_regression(train_aligned, y_train)
print(f'AUC Score: {auc}')


# 预测测试集概率
yprob_logistic = logistic_model.predict_proba(test_aligned)[:, 1]

# 创建结果 DataFrame
result_logistic = pd.DataFrame({
    'ID': test_all['Idx'],
    'Prediction': yprob_logistic
    })

# 保存结果
result_logistic.to_csv('result_logistic_regression.csv', encoding='utf-8', index=False)
print("逻辑回归预测结果已保存到 result_logistic_regression.csv")