...
class QIN(BaseModel):
    def __init__(self,
                 ...
                 learning_rate=1e-3,
                 embedding_dim=10,
                 embedding_dropout=0.2,  # Embedding Dropout
                 net_dropout=0.1,
                 batch_norm=False,
                 ...
                 attention_num_heads=4,  # 多头数
                 **kwargs):

        super(QIN, self).__init__(feature_map,
                                             model_id=model_id,
                                             gpu=gpu,
        ...
        self.attention_layers = MultiHeadTargetAttention(
            ...
            attention_dim=self.item_info_dim,
    ...
    def forward(self, inputs):
        ...
        if batch_dict:
            ...
            feature_emb = self.embedding_dropout_layer(feature_emb)
            emb_list.append(feature_emb)
        item_feat_emb = self.embedding_layer(item_dict, flatten_emb=True)
        item_feat_emb = self.embedding_dropout_layer(item_feat_emb)
...
class MultiHeadTargetAttention(nn.Module):
    def __init__(self, input_dim=64, attention_dim=64, num_heads=4, use_scale=True):
    ...
    def forward(self, target_item, history_sequence, mask=None):
        ...
        Q = self.W_q(target_item).view(B, self.num_heads, self.head_dim).transpose(0,1)  # (H,B,D)
        K = self.W_k(history_sequence).view(B, seq_len, self.num_heads, self.head_dim).permute(2,0,1,3)  # (H,B,L,D)
        V = self.W_v(history_sequence).view(B, seq_len, self.num_heads, self.head_dim).permute(2,0,1,3)  # (H,B,L,D)
        ...
        for i in range(self.num_heads):
            ...
            if m is not None:
                m = m.view(B,1,-1)
            o = self.dot_attention(q, k, v, scale=self.scale, mask=m)
            outputs.append(o.squeeze(1))
        concat = torch.cat(outputs, dim=-1)
        out = self.W_o(concat) + target_item
        return out
...
# Quadratic神经网络（激活用GELU）
class QuadraticNeuralNetworks(nn.Module):
    def __init__(self, input_dim, num_layers=5, net_dropout=0.1, num_row=2, batch_norm=False):
        ...
        for i in range(num_layers):
            ...
            self.activation.append(nn.GELU())
...
class QuadraticLayer(nn.Module):
    ...
    def forward(self, x):
        h = self.linear(x).view(-1, self.num_row, self.input_dim)
